...
const drawTable = (list) => {
    ...
    list.forEach(item => {
        ...
        htmlOut = htmlOut + `<td><input type="button" class="btn btn-danger btn-sm" value="delete" onclick="delUser('${item.email}')" /></td>`;
        htmlOut = htmlOut + '</tr>';
    });
...
// Agregar usuario desde un input
...
//editar segun el email y cargar datos a los inputs
...
//CLASE 18
const editAcction = () => {
    // esto es igual que poner esto: document.getElementById('email').disabled = document.getElementById('email').disabled === true ? false : true;
    
    document.getElementById('email').disabled = document.getElementById('email').disabled ? false : true; //operador terniario
...
//OCULTAR BOTON DE CREAR TABLA
const ocultarCrear = () => {
    document.getElementById('createTable').style.display = document.getElementById('createTable').style.display === '' ? 'none' : 'none';
}